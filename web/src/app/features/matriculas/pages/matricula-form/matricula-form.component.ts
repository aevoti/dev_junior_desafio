 ...
 * TODO: trocar o campo "Nível do Pokémon" por um select real de Pokémons do Treinador.
 */
@Component({
...
export class MatriculaFormComponent implements OnInit {
  ...
  get planoExigeNivelMinimo(): boolean {
    const plano = this.planos.find((p) => p.id === this.planoTreinamentoId);
    return !!plano && plano.nivelMinimoPokemon > 1;
  }

  get nivelInsuficiente(): boolean {
    const plano = this.planos.find((p) => p.id === this.planoTreinamentoId);
    if (!plano || this.nivelPokemon === null) return false;
    return this.nivelPokemon < plano.nivelMinimoPokemon;
  }
